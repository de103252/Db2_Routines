...
is_numeric(unsigned char *data, size_t datalen) {
  ...
  for (i = 0; i < datalen - 1; i++) {
    if ((dataÝi¨ & 0xF0) > 0x90) return false;
    if ((dataÝi¨ & 0x0F) > 0x09) return false;
  }
  if ((dataÝi¨ & 0xF0) > 0x90) return false;
  if ((dataÝi¨ & 0x0F) < 0x0A) return false;
  return true;
}
                      ...
                      sqlint32       flags)
{
  ...
  for (int i = 0; i < otdesc->numtfcol; i++) {
    void *coldata = otdesc->coldataÝi¨;
    int colind = 0;
    struct sqlchar *chardata;
    sqluint32 datalen = otdesc->column_infoÝi¨.u.datalen;
    switch (otdesc->column_infoÝi¨.datatype & ~1) {
      case SQL_TYP_CHAR:
      case SQL_TYP_BINARY:
      ...
      case SQL_TYP_BIGINT:
        if (buflen_ok(offset, datalen, reclen)) {
          memcpy(otdesc->coldataÝi¨, buf + offset, datalen);
        } else if (flags & NULL_ON_SHORT_BUF) {
          colind = -1;
        } else {
          signal_short_buffer(recno, i, offset, sqlstate, msgtext);
      ...
      case SQL_TYP_NUMERIC:
        datalen = (otdesc->column_infoÝi¨.u.declen.precision + 1) / 2;
        if (buflen_ok(offset, datalen, reclen)) {
      ...
      case SQL_TYP_TIME:
        datalen = 4;
        if (buflen_ok(offset, datalen, reclen)) {
          char timestrÝ9¨ = "HH.MM.00";
          memcpy(timestr, buf + offset, 2);
          memcpy(timestr + 3, buf + offset + 2, 2);
          if (is_time(timestr)) {
            memcpy(otdesc->coldataÝi¨, timestr, sizeof timestr - 1);
          } else if (flags & NULL_ON_INVALID_DATA) {
            colind = -1;
          } else {
            signal_invalid_data(recno, i, offset, sqlstate, msgtext);
